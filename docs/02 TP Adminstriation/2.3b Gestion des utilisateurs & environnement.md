...
> [!NOTE] Un shell, c'est quoi ?
> Un shell est un programme utilisateur permettant de saisir des commandes et d’en demander l’exécution au système.

Lorsqu’une session utilisateur est ouverte, le système met en place un **environnement de base** : identité de l’utilisateur, répertoire personnel et un ensemble initial de variables d’environnement. Cet environnement existe **avant** le lancement du shell et peut être utilisé par n’importe quel programme associé au compte.

Lorsque le programme associé au compte est un shell interactif (sh, bash ksh), celui-ci s’exécute **dans cet environnement déjà existant**. Lors de démarrage du shell, il **exécute automatiquement certains scripts**, appelés _profils_.  Ces scripts définissent ou modifient des **variables d’environnement** (comme `PATH`, `HOME`, `LANG`, `umask`) qui seront ensuite **héritées par les commandes exécutées depuis ce shell**.

1) Un premier niveau de configuration est **global**. Le fichier `/etc/profile` est exécuté pour tous les utilisateurs lorsqu’un shell de connexion est lancé. Il permet de définir un environnement commun au système.
2) Un second niveau est **spécifique à l’utilisateur**. Le fichier `~/.profile`, s’il existe, est exécuté après `/etc/profile`. Il permet à chaque utilisateur d’ajuster son environnement personnel sans affecter les autres comptes.
