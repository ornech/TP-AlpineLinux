...
Le dossier `/proc` est un **pseudo-système de fichiers géré par le noyau**.  Il ne contient pas de fichiers stockés sur le disque, mais expose **en temps réel** sous la forme de fichiers l’état du système et des processus..
...
Certains signaux proviennent du **terminal** (`Ctrl+C`, `Ctrl+Z`), d’autres sont envoyés explicitement via la commande `kill`. Le noyau transmet le signal au processus quelle que soit son origine. 

| Signal      | N°  | À quoi il sert                                  | Commande associée |
| ----------- | --- | ----------------------------------------------- | ----------------- |
| **SIGTERM** | 15  | Demander à un programme de s’arrêter proprement | `kill <PID>`      |
| **SIGKILL** | 9   | Forcer l’arrêt immédiat (dernier recours)       | `kill -9 <PID>`   |
| **SIGINT**  | 2   | Interrompre depuis le clavier (`Ctrl+C`)        | `kill -2 <PID>`   |
| **SIGSTOP** | 19  | Mettre un programme en pause forcé              | `kill -19 <PID>`  |
| **SIGCONT** | 18  | Reprendre un programme en pause                 | `kill -18 <PID>`  |
Exemples:
```
kill -9  <PID>    # SIGKILL : arrêt forcé
kill -19 <PID>    # SIGSTOP : suspension forcée
kill -18 <PID>    # SIGCONT : reprise
kill -2  <PID>    # SIGINT  : interruption
```
...
> - `kill -9` impose un arrêt brutal **sans nettoyage** (attention aux  verrous, fichiers temporaires ou sockets laissés en l’état).
...
### 2.1.5.c Une interface uniforme pour gérer les services
...
**### 2.1.5.d Un arrêt structuré du système**  
