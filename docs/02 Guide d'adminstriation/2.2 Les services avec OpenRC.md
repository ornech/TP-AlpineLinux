# 2.2 Les services avec OpenRC
## 1. Un programme, c’est quoi ?
Dans un système Unix, un **programme** c'est simplement :
- un fichier exécutable,    
- lancé par un utilisateur,
- qui tourne tant que quelqu’un l’utilise.
...
- démarre quand on le lance,
- s’arrête quand il a fini,
- est lié au terminal qui l’exécute.
## 2. Un démon (daemon), c’est autre chose ...

Certains programmes doivent fonctionner **en continu** pour fournir une fonctionnalité au système ou aux utilisateurs.
...
## 3. Le démarrage d’un système Linux
...
![[2.2 Les services avec OpenRC.png]]
## 4. Pourquoi orchestrer les démons ?
...
> [!danger]
> 
> - Modifier un script dans `/etc/init.d` (erreur classique).
> - Activer (`rc-update`) un service non installé.
> - Croire que `rc-update add` démarre le service (il **n’arrive qu’au prochain boot**).    
> - Oublier d’activer un service, puis s’étonner qu’il n’est pas lancé au redémarrage.
>     
