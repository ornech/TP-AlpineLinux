...
# BusyBox : la brique essentielle d’Alpine Linux

_(version courte pour diapo ou support pédagogique)_

**BusyBox est un programme unique qui regroupe la plupart des commandes de base d’un système Linux** : `ls`, `cp`, `mv`, `ps`, `adduser`, `mount`, etc.  
Au lieu d’avoir un binaire séparé pour chaque commande, Alpine utilise BusyBox comme un **“couteau suisse” minimaliste**, extrêmement léger.

Ce choix permet à Alpine Linux d’être :

- **très compact** (quelques Mo seulement)
    
- **rapide à démarrer**
    
- **simple à maintenir**
    
- **plus sécurisé**, car il y a moins de code et moins de services par défaut
    

> En résumé : **BusyBox fournit les outils essentiels du système dans un seul programme, ce qui fait d’Alpine une distribution minimaliste, efficace et parfaitement adaptée à l’apprentissage des bases de Linux.**