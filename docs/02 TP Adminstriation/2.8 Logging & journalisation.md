...
En effet, les programmes n’écrivent pas chacun leurs logs “à leur façon”.  Sur un système compatible **POSIX** comme Alpine, les messages de journalisation sont pris en charge syslogd (interface standardisé pour écrire un log), ce qui garantit un format de log cohérent et uniforme, quel que soit le programme à l’origine du message.
...
On peut également modifié le niveau de priorité du message ave l'option t
```
logger -t test_log2 -p 4 "Test de log : avec niveau WARNING"
logger -t test_log3 -p 0 "Test de log : avec niveau EMERGENCY"
```
...
Jusqu’ici, on a constaté que les logs respectent une norme et qu’ils sont géré par `syslogd`.  On a même généré quelques messages pour vérifier ce mécanisme.  Mais la question de la machine à café reste entière:
...
SYSLOGD_OPTS=""
```
En supprimant l’option `-C`, on impose à `syslogd` d’écrire les logs sur le disque et non plus en mémoire. Syslogs est désormais configuré pour écrire les logs dans le fichier /var/log/messages.
...
Les étudiants doivent travailler **uniquement** sur les services suivants :

- `sshd`  (authentification distante)
- `login` (authentification locale)
### Niveau de sévérité

Pour chacun des deux services :

- choisir un **niveau de sévérité** parmi :
...
### Attendu
- identification des messages liés à `sshd` et à `login` dans les logs ;
...
### Question bonus
Expliquez le rôle de `logrotate` dans un système Linux.
Précisez :
