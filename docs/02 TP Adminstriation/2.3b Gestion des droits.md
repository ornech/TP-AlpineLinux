...
Une question pertinente à traiter maintenant serait : **« Quel est le niveau de droits appliqué par défaut lors de la création d’un fichier ? »**  
Autrement dit : lorsqu’un utilisateur crée un fichier, existe-t-il un risque que les niveaux d’accès souhaités ne soient pas respectés ?

Faisons le test. Connectez-vous avec un compte _développeur_ et créez un fichier dans `/srv/dev/` :

On constate que les fichiers apparaissent en **rw-rw-r--**, ce qui est **trop permissif** pour un répertoire censé rester privé au groupe.
...
- une **notation symbolique**, comme `rw-rw-r--`, pensée pour l’humain car elle montre d’un coup d’œil qui peut lire, écrire ou exécuter ;
...
Cette notation octale s’explique par la structure des permissions : chaque droit (r, w, x) correspond à **un bit** auquel une valeur est associée :
-  le premier bit `x`(exécution)  vaut **1**
- le second bit `w` (écriture) vaut **2**
- le troisième bit `r`(lecture) vaut **4**

Lorsque l’on active un bit, sa valeur s’ajoute au total du trio. Par exemple :
- activer seulement `x` donne **1**
- activer seulement `w` donne **2**
- activer seulement `r` donne **4**

- activer `r` et `w` donne 4 + 2 = **6**
- activer `r` et `x` donne 4 + 1 = **5**
- activer `w` et `x` donne 2 + 1 = **3**
- activer `r`, `w` et `x` donne 4 + 2 + 1 = **7**

Un trio de permissions peut donc produire **8 valeurs possibles**, de 0 (`---`) à 7 (`rwx`).  
Comme ces huit valeurs se représentent naturellement par **un seul chiffre en base 8**, Unix a choisi l’octal pour coder les droits : la valeur octal décrit un trio r/w/x, et trois bits successifs décrivent propriétaire, groupe et autres.

À ces trois catégories classiques — propriétaire, groupe et autres — s’ajoutent enfin trois **bits spéciaux** (SUID, SGID et Sticky), destinés à régler certains comportements particuliers que les permissions ordinaires ne permettent pas d’exprimer.
